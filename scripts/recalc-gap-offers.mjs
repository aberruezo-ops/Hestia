...
const autoDiscount = (season, daysUntil) => {
  ...
  if (season === 'critica' && daysUntil <= 7) return 25;
  if (season === 'alta' && daysUntil <= 14) return 15;
  if (daysUntil <= 30) return 20;
  return null; // todavía no es última hora
};

// ── huecos cortos vigentes por apt ───────────────────────────────────────────
...
// 2) AUTO-CREA — huecos cortos vigentes SIN oferta → descuento última hora
for (const [key, g] of Object.entries(shortGaps)) {
  if (next[key]) continue; // ya tiene oferta (manual o auto vigente) → respetar
  const season = dominantSeason(g.start, g.end);
  const daysUntil = diff(today, g.start);
  const value = autoDiscount(season, daysUntil);
  if (value == null) continue; // todavía lejos → sin oferta
...
if (!removed.length && !created.length) { console.log('✓ Ofertas de hueco al día. Sin cambios.'); process.exit(0); }
if (DRY) { console.log('\n(dry-run — no se escribe nada)'); process.exit(0); }

prices.gapOverrides = next;
fs.writeFileSync(PRICES, JSON.stringify(prices, null, 2) + '\n');
console.log(`\nprices.json actualizado (−${removed.length} / +${created.length}).`);
