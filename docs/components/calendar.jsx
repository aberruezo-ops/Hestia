    ...
    <div className="req-panel" style={{ '--req-accent': accent }}>
        ...
        <div className="price-engine">
          <div className="price-main-row">
            <div className="price-direct-block">
              ...
              <AnimatedPrice className="price-direct-total" value={calc.directTotal} format={fmt} />
      ...
      <div className="req-guests">
        ...
        <label className="req-pets-toggle">
          ...
          <span><HiIcon name="paw" size={15} style={{ verticalAlign: '-2px', marginRight: 3 }} />{lang === 'es' ? 'Mascota' : 'Pet'}</span>
        </label>
        <label className="req-pets-toggle">
          <input type="checkbox" checked={baby} onChange={e => setBaby(e.target.checked)}/>
          <span><HiIcon name="baby" size={15} style={{ verticalAlign: '-2px', marginRight: 3 }} />{lang === 'es' ? 'Bebé' : 'Baby'}</span>
        </label>
      </div>
...
const AptCalendar = ({ aptId, lang, accent }) => {
  ...
  return (
    <section id="apt-avail" className="apt-avail on-dark" data-avail-checker style={sectionStyle}>
      ...
      {isDemo && (
        <div className="avail-demo">
          <HiIcon name="spark" size={15} style={{ verticalAlign: '-2px', marginRight: 4 }} />
          {lang === 'es'
      ...
      {!isDemo && hasSyncError && (
        <div className="avail-sync-err">
          {lang === 'es'
            ? <><HiIcon name="alert" size={15} style={{ verticalAlign: '-2px', marginRight: 4 }} />La sincronización automática no pudo conectar. Las fechas mostradas pueden no estar actualizadas. Escríbenos para confirmar disponibilidad: <a href="https://wa.me/34620316370" target="_blank" rel="noopener">WhatsApp →</a></>
            : <><HiIcon name="alert" size={15} style={{ verticalAlign: '-2px', marginRight: 4 }} />Auto-sync could not connect. Dates shown may not be up to date. Write to us to confirm availability: <a href="https://wa.me/34620316370" target="_blank" rel="noopener">WhatsApp →</a></>}
        </div>
      )}
