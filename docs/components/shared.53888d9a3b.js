...
function _hestiaFindAlternatives({
  ...
  const isFree = (id, cin, cout) => {
    const blk = avail[id] && avail[id].blocked;
    if (!blk) return false;
    return !blk.some(r => cin < r.end && cout > r.start);
  };
  const out = [];
  const seen = new Set();
  const perAptN = {};
  ...
  Object.values(byApt).forEach(list => list.sort((a, b) => Math.abs(a.shiftDays) - Math.abs(b.shiftDays) || a.total - b.total));
  Object.values(byApt).forEach(list => list.forEach((o, i) => {
    o._rank = i;
  }));
  out.sort((a, b) => a._rank - b._rank || Math.abs(a.shiftDays) - Math.abs(b.shiftDays) || a.total - b.total);
  out.forEach(o => {
    delete o._rank;
  });
