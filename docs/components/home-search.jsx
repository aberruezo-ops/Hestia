...
const HsDateRange = ({ checkin, checkout, setCheckin, setCheckout, avail, apt, lang, today }) => {
  ...
  React.useEffect(() => {
    ...
    if (!visible) { setViewY(cy); setViewM(cm - 1); }
  }, [checkin]);
              ...
              <div className="hs-price-block">
                <div className="hs-pb-main">
                  <div className="hs-pb-direct">
                    ...
                    <AnimatedPrice className="hs-pb-total"
                      value={isLsStay && lsCalc ? lsCalc.total : calc.directTotal} format={fmt} />
...
const HomeSearch = ({ lang, b2b = false }) => {
  ...
  const handleNotify = async (e) => {
    ...
    try {
      ...
      if (window._hestiaSendLeadFd) await window._hestiaSendLeadFd(fd);
      else await fetch('https://api.web3forms.com/submit', { method: 'POST', body: fd });
      setNotifyState('sent');
    } catch (_) {
      setNotifyState('idle');
    }
      ...
      <div className="hs-inner">
        ...
        <form className="hs-form" onSubmit={handleSearch} noValidate>
          ...
          <HsDateRange
            ...
            setCheckin={v => { userTouchedRef.current = true; setCheckin(v); setResults(null); }}
            setCheckout={v => { userTouchedRef.current = true; setCheckout(v); setResults(null); }}
            avail={avail} apt={apt}
          ...
          <div className="hs-submit-row">
            ...
            {loading && (
              <IndaloLoader className="hs-loading-note" size={30}
                label={lang === 'es' ? 'Cargando disponibilidad…' : 'Loading availability…'} />
            )}
          </div>
          <div className="hs-trust">
            <span><HiIcon name="lock" size={14} style={{ verticalAlign: '-2px', marginRight: 3 }} />{lang === 'es' ? 'Sin comisiones' : 'No fees'}</span>
          ...
          <div className="hs-results" id="hs-results">
            ...
            {b2b && results.some(r => r.available === true) && (
              <div className="hs-longstay-nudge hs-b2b-nudge">
                <div className="hs-ls-icon"><HiIcon name="briefcase" size={22} /></div>
            ...
            {results._isLongStay && !b2b && (
              <div className="hs-longstay-nudge">
                <div className="hs-ls-icon"><HiIcon name="home" size={22} /></div>
                <div className="hs-ls-body">
            ...
            <>
              ...
              {notifyState === 'sent' ? (
                <p className="hs-notify-ok">
                  {lang === 'es' ? 'Anotado. Te escribimos si se libera algo.' : 'Got it. We\'ll message you if something opens up.'}
                </p>
              ) : (
