...
const AdminApp = () => {
  ...
  return (
    <div className="pe-shell">
      ...
      {mode === 'inteligencia' ? <IntelligenciaTab token={token} onNavigate={tab => { setMode(tab); setError(null); setSuccess(null); }} /> : mode === 'contract' ? <ContractTab pricesData={data} prefill={contractPrefill} /> : mode === 'prereservas' ? <PrereservasTab token={token} /> : mode === 'reservas' ? <ReservasTab token={token} onOpenContract={r => { setContractPrefill(r); setMode('contract'); }} /> : mode === 'leila' ? <LeilaTab token={token} /> : mode === 'facturas' ? <FacturasTab token={token} /> : mode === 'reviews' ? renderReviewsTab() : (
      <>
      <div className="pe-card">
