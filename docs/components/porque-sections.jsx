...
const PORQUE_COPY = {
  es: {
    ...
    origin_title: (<>Empezó en 2016,<br/><em>con dos personas y tres llaves.</em></>),
    ...
    name_title: (<>Hestía, la diosa<br/><em>que guarda el fuego de casa.</em></>),
    ...
    values: [
      { letter: 'H', name: 'Hospitalidad', desc: 'La llama que recibe, la que se honra al llegar y al partir. Tu estancia no acaba el día que te vas: acaba el día que quieres volver.' },
      { letter: 'E', name: 'Escucha',      desc: 'Personas, no clientes. Alex habla español, Fran inglés, y los dos conocen tu nombre antes de que cruces la puerta porque escuchan, sin formularios de por medio.' },
      { letter: 'S', name: 'Sencillez',    desc: 'Sin oficina ni recepción: tres llaves, tres casas y dos personas al otro lado del teléfono.' },
      { letter: 'T', name: 'Transparencia', desc: 'Precios claros, fotos reales y distancias medidas en metros. Si algo no está, lo decimos; si algo se rompe, lo arreglamos.' },
      { letter: 'I', name: 'Integridad',   desc: 'Diseñamos cada Hestía como si fuera nuestra propia casa, porque en cierto modo lo es: el cojín bien puesto, el café listo, la toalla doblada antes de que llegues.' },
      { letter: 'A', name: 'Arraigo',      desc: 'Décadas en Vera Playa. Hestía huele a sal, a olivar y a calima del Sahara porque el sitio es real, no un decorado.' },
    ],
    values_closing: (<>Seis valores, <em>una sola confianza.</em></>),
    ...
    travelers: [
      ...
      { icon: '🌿', t: 'Deja el sitio mejor de como lo encontró.', d: 'Recomienda el bar de toda la vida, respeta el silencio de la tarde, cuida el entorno.' },
      { icon: '🔄', t: 'Vuelve, y trae a alguien.', d: 'Cuando encuentra un lugar donde se sintió en casa, vuelve, y convierte a otros en ese mismo tipo de huésped.' },
    ],

    evol_eyebrow: 'La transformación',
    evol_title: (<>Diez años después,<br/><em>una marca que dice quiénes somos.</em></>),
  ...
  en: {
    ...
    origin_title: (<>It started in 2016,<br/><em>with two people and three keys.</em></>),
    ...
    name_title: (<>Hestía, the goddess<br/><em>who keeps the home fire.</em></>),
    ...
    values: [
      { letter: 'H', name: 'Hospitality',  desc: 'The flame that welcomes, honoured on arrival and on leaving. Your stay does not end the day you leave: it ends the day you want to come back.' },
      { letter: 'E', name: 'Listening',    desc: 'People, not clients. Alex speaks Spanish, Fran English, and both know your name before you cross the door because they listen, no forms involved.' },
      { letter: 'S', name: 'Simplicity',   desc: 'No office, no reception desk: three keys, three homes, two people on the other end of the phone.' },
      { letter: 'T', name: 'Transparency', desc: 'Clear prices, real photos, distances measured in metres. If something is missing, we say so; if something breaks, we fix it.' },
      { letter: 'I', name: 'Integrity',    desc: 'We design every Hestía as if it were our own home, because in a way it is: the cushion in place, the coffee ready, the towel folded before you arrive.' },
      { letter: 'A', name: 'Authenticity', desc: 'Decades in Vera Playa. Hestía smells of salt, olive grove and Saharan calima, because the place is real, not a stage set.' },
    ],
    values_closing: (<>Six values, <em>one trust.</em></>),
    ...
    travelers: [
      ...
      { icon: '🌿', t: 'Leaves the place better than they found it.', d: 'They recommend the local bar, respect the quiet of the afternoon, look after their surroundings.' },
      { icon: '🔄', t: 'They come back, and bring someone.', d: 'When they find a place where they felt at home, they return, and turn others into that same kind of guest.' },
    ],

    evol_eyebrow: 'The transformation',
    evol_title: (<>Ten years on,<br/><em>a brand that says who we are.</em></>),
...
const PorqueValores = ({ lang }) => {
  ...
  const onKeyNav = (e) => {
    if (e.key === 'ArrowRight') { e.preventDefault(); setActiveIdx(i => (i + 1) % COUNT); }
    else if (e.key === 'ArrowLeft')  { e.preventDefault(); setActiveIdx(i => (i - 1 + COUNT) % COUNT); }
    else if (e.key === 'Home')       { e.preventDefault(); setActiveIdx(0); }
    else if (e.key === 'End')        { e.preventDefault(); setActiveIdx(COUNT - 1); }
  };

  return (
    <section className="pq-valores pq-valores-v4 section-dark">
      <div className="container pq-v4-intro">
      ...
      >
        {/* Stack de fotos detrás: un detalle de Hestía por letra,
            blur + opacidad bajos. Crossfade con activeIdx. */}
        <div className="pq-v4-photo-stack" aria-hidden="true">
          {t.values.map((vv, i) => (
            <div
              key={i}
        ...
        <div className="pq-v4-wash-stack" aria-hidden="true">
          {t.values.map((vv, i) => (
            <div
              key={i}
        ...
        <div className="pq-v4-grain" aria-hidden="true"/>

        <div className="pq-v4-main">
          <div className="pq-v4-letter" aria-hidden="true">
            {t.values.map((vv, i) => (
              <span
                key={i}
          ...
          <div className="pq-v4-text" key={activeIdx}>
            <div className="pq-v4-text-meta">
              <span className="pq-v4-text-idx">
                0{activeIdx + 1}<span className="pq-v4-text-idx-sep"> / </span>0{COUNT}
              </span>
              <span className="pq-v4-text-chip">{v.letter}</span>
            </div>
            <h3 className="pq-v4-text-name" aria-label={v.name}>
              {nameChars.map((ch, i) => (
                <span
                  key={i}
                  className="pq-v4-name-char"
                  style={{ '--ch-delay': `${i * 26}ms` }}
                >
                  {ch === ' ' ? ' ' : ch}
                </span>
              ))}
            </h3>
            <p className="pq-v4-text-desc">{v.desc}</p>
          </div>
        </div>
        ...
        >
          {t.values.map((vv, i) => (
            <button
              ...
              style={{ '--hue': PORQUE_V4_HUES[vv.letter] || '212, 168, 74' }}
              onClick={() => onSelect(i)}
              aria-label={`${vv.letter}, ${vv.name}`}
            >
              <span className="pq-v4-strip-letter">{vv.letter}</span>
              {i === activeIdx && !paused && (
                <span className="pq-v4-strip-ring" key={activeIdx} aria-hidden="true">
                  <svg viewBox="0 0 24 24" className="pq-v4-strip-ring-svg">
                    <circle cx="12" cy="12" r="10.5" className="pq-v4-strip-ring-track"/>
                    <circle cx="12" cy="12" r="10.5" className="pq-v4-strip-ring-fill"/>
                  </svg>
                </span>
              )}
            </button>
      ...
      <div className="container pq-v4-outro">
        ...
        <div className="nos-viajero-grid">
          {t.travelers.map((v, i) => (
            <div key={i} className="nos-viajero-card reveal" style={{ transitionDelay: `${i * 0.12}s` }}>
              <div className="nos-viajero-icon"><EmojiIcon emoji={v.icon} size={26} /></div>
