...
const FloatingChat = ({ lang }) => {
  ...
  return (
    <div className={`float-chat ${open ? 'open' : ''}${giftOpen ? ' gift-open' : ''}`}>
      ...
      <button className="gift-fab" onClick={() => { setGiftOpen(!giftOpen); setOpen(false); }}
      ...
      <button className="bubble-btn" onClick={() => { setOpen(!open); setGiftOpen(false); }} aria-label={open ? (lang === 'es' ? 'Cerrar chat' : 'Close chat') : (lang === 'es' ? 'Abrir chat' : 'Open chat')}>
