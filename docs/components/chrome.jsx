...
const Header = ({ mode, scrolled, lang }) => {
  ...
  React.useEffect(() => {
    const sync = () => {
      ...
      try { setVitMin(sessionStorage.getItem('hestia-vit-min') !== '0'); } catch (_) {}
    };
...
const FloatingChat = ({ lang }) => {
  ...
  const giftWaText = encodeURIComponent(lang === 'es'
    ? 'Hola, me gustaría regalar una experiencia Hestía (luna de miel, cumpleaños, aniversario, reunión especial...). Os cuento mi caso (la ocasión, fechas aproximadas y para quién). ¿Me contáis opciones, incluido el cupón o bono digital? '
    : 'Hi, I would like to gift a Hestía experience (honeymoon, birthday, anniversary, special gathering...). Here is my case (the occasion, approximate dates and who it is for). Could you tell me the options, including the digital voucher? ');
  const giftSubject = encodeURIComponent(lang === 'es' ? 'Regalar una experiencia Hestía' : 'Gifting a Hestía experience');
  const giftBody = encodeURIComponent(lang === 'es'
  ...
  return (
    <div className={`float-chat ${open ? 'open' : ''}${giftOpen ? ' gift-open' : ''}`}>
      <div className="gift-panel" role="dialog" aria-hidden={!giftOpen} aria-labelledby="gift-panel-title">
        <h4 id="gift-panel-title">{lang === 'es' ? 'Regala Hestía' : 'Gift Hestía'}</h4>
        <p className="small gift-lead">
          {lang === 'es'
            ? 'Regala una experiencia Hestía y te ayudamos a prepararla en aquello que podamos.'
            : 'Gift a Hestía experience and we help you prepare it however we can.'}
        </p>
        <ul className="gift-occasions">
          <li><HiIcon name="heart" size={13} className="gift-occ-ic" /> {lang === 'es' ? 'Lunas de miel' : 'Honeymoons'}</li>
          <li><HiIcon name="spark" size={13} className="gift-occ-ic" /> {lang === 'es' ? 'Cumpleaños' : 'Birthdays'}</li>
          <li><HiIcon name="star-fill" size={13} className="gift-occ-ic" /> {lang === 'es' ? 'Aniversarios' : 'Anniversaries'}</li>
          <li><HiIcon name="cocktail" size={13} className="gift-occ-ic" /> {lang === 'es' ? 'Reuniones especiales' : 'Special gatherings'}</li>
        </ul>
        <p className="small gift-bono">
          <HiIcon name="ticket" size={15} className="gift-bono-ic" />
          {lang === 'es'
            ? 'Si quieres, te lo enviamos como cupón o bono digital, equivalente a una estancia.'
            : 'If you like, we send it as a digital voucher, equal to a stay.'}
        </p>
        <p className="small gift-indalo">
          <HiIcon name="indalo" size={17} className="gift-indalo-ic" />
          {lang === 'es'
            ? 'El Indalo, amuleto de Almería, dicen que solo trae suerte si te lo regalan.'
            : 'The Indalo, Almería\'s good-luck charm, is said to work only when given as a gift.'}
        </p>
        <p className="small gift-cta-line">{lang === 'es' ? 'Cuéntanos y te contamos. Sin compromiso.' : 'Tell us and we will tell you. No obligation.'}</p>
        <div className="gift-actions">
          <a className="cp-btn cp-btn-wa" href={`https://wa.me/34620316370?text=${giftWaText}`} target="_blank" rel="noopener">
            <WaIcon/> {lang === 'es' ? 'Cuéntanos por WhatsApp' : 'Tell us on WhatsApp'}
          </a>
          <a className="cp-btn cp-btn-tel" href={`mailto:info@hestiayourhome.com?subject=${giftSubject}&body=${giftBody}`}>
            {lang === 'es' ? 'Por email' : 'By email'}
          </a>
        </div>
      </div>
      <button className="gift-fab" onClick={() => setGiftOpen(!giftOpen)}
              aria-expanded={giftOpen} aria-label={lang === 'es' ? 'Regala Hestía' : 'Gift Hestía'}>
        <HiIcon name="gift" size={22} /> <span>{lang === 'es' ? 'Regala Hestía' : 'Gift Hestía'}</span>
      </button>
      ...
      <button className="bubble-btn" onClick={() => setOpen(!open)} aria-label={open ? (lang === 'es' ? 'Cerrar chat' : 'Close chat') : (lang === 'es' ? 'Abrir chat' : 'Open chat')}>
...
const IndaloGrabadoMark = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <g opacity="0.28" transform="translate(0.5 0.35)" stroke="var(--ber-dk)" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round">
      <path d={IND2_HEAD} fill="var(--ber-dk)" stroke="none"/>
      <path d={IND2_BODY}/><path d={IND2_ARMS}/><path d={IND2_LEGS}/>
    </g>
    <g stroke="var(--vt-dk)" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round">
      <path d={IND2_HEAD} fill="var(--vt-dk)" stroke="none"/>
      <path d={IND2_BODY}/><path d={IND2_ARMS}/><path d={IND2_LEGS}/>
    </g>
  </svg>
);
...
const IndaloAcuarelaMark = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    <g stroke="var(--vm2)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" opacity="0.9">
      ...
      <path d="M11.9 14.5l-2 4M12.1 14.5l4.5 5.2"/>
    </g>
  </svg>
);
const IndaloAzulejoMark = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    <g stroke="#5B9BC4" strokeWidth="2.1" strokeLinecap="butt" strokeDasharray="0.9 0.55">
      <path d={IND3_HEAD}/><path d={IND3_ARCH}/><path d={IND3_BODY}/><path d={IND3_ARMS}/><path d={IND3_LEGS}/>
    </g>
    <g stroke="#F0CE7A" strokeWidth="0.75" strokeLinecap="butt" strokeDasharray="0.9 0.55" strokeDashoffset="0.7">
      <path d={IND3_HEAD}/><path d={IND3_ARCH}/><path d={IND3_BODY}/><path d={IND3_ARMS}/><path d={IND3_LEGS}/>
    </g>
  </svg>
);
const IndaloForjaMark = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    <g stroke="#3A3A3A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d={IND4_LOOP}/>
      <path d={IND4_TORSO}/><path d={IND4_ARM_UP}/><path d={IND4_ARM_DN}/>
      <path d={IND4_LEG_BACK}/><path d={IND4_LEG_FWD}/>
    </g>
    <g fill="#3A3A3A">
      <circle cx="11.3" cy="15" r="0.55"/>
      <circle cx="13" cy="6.8" r="0.5"/>
      <circle cx="18.9" cy="7.7" r="0.5"/>
      <circle cx="6.2" cy="21" r="0.5"/>
      <circle cx="16.4" cy="17.7" r="0.5"/>
    </g>
  </svg>
);
...
const IndaloJoyaMark = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <circle cx="12" cy="12" r="10.5" fill="#1a0f1e"/>
    <circle cx="12" cy="12" r="9.2" fill="none" stroke="#F0CE7A" strokeWidth="0.25" opacity="0.5"/>
    <g stroke="#F0CE7A" strokeWidth="0.55" strokeLinecap="round" strokeLinejoin="round">
      <path d="M6.5 11.5C6.5 6.5 17.5 6.5 17.5 11.5"/>
      <path d="M12 8V14.5"/>
      <path d="M12 10.8 6.5 11.5M12 10.8 17.5 11.5"/>
      <path d="M12 14.5 9.8 19M12 14.5 14.2 19"/>
    </g>
    <g fill="#F0CE7A">
      <circle cx="12" cy="8" r="0.5"/>
      <circle cx="6.5" cy="11.5" r="0.5"/>
      <circle cx="17.5" cy="11.5" r="0.5"/>
      <circle cx="9.8" cy="19" r="0.45"/>
      <circle cx="14.2" cy="19" r="0.45"/>
    </g>
  </svg>
);
const INDALO_STYLES = [
...
const IndaloGallery = ({ lang }) => (
  <div className="footer-indalo-gallery">
    <span className="fig-eyebrow">
      {lang === 'es' ? 'El Indalo, nuestro amuleto' : 'The Indalo, our charm'}
    </span>
...
const Footer = ({ lang }) => {
  ...
  return (
    <footer className="on-dark">
      ...
      <IndaloWatermark className="footer-indalo-wm" />
      <div className="footer-grid">
        <div className="col footer-brand">
          ...
          <div className="footer-amuleto">
            <IndaloShape className="indalo-seal" width="26" height="26" />
            <span>
              {lang === 'es'
                ? 'Hestía, tu amuleto de hogar en Vera Playa.'
                : 'Hestía, your home charm in Vera Playa.'}
            </span>
          </div>
        </div>
        <div className="col">
        ...
        <div className="col">
          ...
          <ul>
            ...
            <li><a href="normas.html">{lang === 'es' ? 'Normas de la casa' : 'House rules'}</a></li>
      ...
      <div className="footer-bottom">
        ...
        <a className="footer-owner-link" href="p-edit.html">
