...
const COPY = {
  es: {
    ...
    manifest_title: (<>Hestía se vive, <em>y se cuida para quien venga después.</em></>),
    manifest_lead_1: 'No vienes a hacer turismo. Vienes a cocinar tu desayuno, a leer en la terraza, a seguir tu ritmo. Durante tu estancia, esto es tu casa.',
  ...
  en: {
    ...
    manifest_title: (<>Hestía is lived in, <em>and cared for by whoever comes next.</em></>),
    manifest_lead_1: 'You’re not here as a tourist. You’re here to cook your breakfast, read on the terrace, keep your own rhythm. While you stay, this is your home.',
...
const _regularStayTotal = (selStart, selEnd, aptId) => {
  ...
  return { baseTotal, stayD, stayDiscAmt, afterStay: baseTotal - stayDiscAmt };
};
...
const _calcStay = (selStart, selEnd, aptId, withPets, guests) => {
  ...
  const applicable = _gapBlocked ? [] : allGov.filter(g =>
    g.apt === aptId && g.type && g.type !== 'none' && g.start >= selStart && g.end <= selEnd);
  const _seasonBaseAt = (ds) => Math.round(((v2 && v2.apts && v2.apts[aptId] && v2.apts[aptId].base) || 0)
    * ((v2 && v2.seasons && v2.seasons[_v2SeasonForDate(ds, v2)] && v2.seasons[_v2SeasonForDate(ds, v2)].multiplier) || 1));
  const _govRate = (g) => {
  ...
  if (applicable.length > 0) {
    ...
    let total = 0, allOverride = true, uniformRate = null, uniform = true, d = selStart;
    for (let i = 0; i < nights; i++) {
    ...
    if (allOverride && uniform) { isGapOffer = true; gapPerNight = uniformRate; }
  } else {
    const reg = _regularStayTotal(selStart, selEnd, aptId);
    fBaseTotal = reg.baseTotal; fStayD = reg.stayD; fStayDiscAmt = reg.stayDiscAmt; fAfterStay = reg.afterStay;
  }
...
Object.assign(window, { HestiaLogoMark, IconSprite, HiIcon, emojiHi, EmojiIcon, AnimatedPrice, lqipFor, BlurImg, WatermarkBadge, IndaloWatermark, IndaloLoader, IndaloShape, Wordmark, COPY, useScrollMode, useReveal, useSectionGlow, BRIDGE_PALETTE, QuickFAQ, SabiasQue, FraseHogar, StickyFacts, _HOME_FACTS_POOL, HESTIA_PRICES, STAY_DISCOUNTS, PET_SUPP_FLAT, _dayPrice, _calcStay, _regularStayTotal, _dayPriceV2, _v2SeasonForDate, _v2BumpedSeasonForDate, _vt, DateRangePicker, _drAvail, _drAdj, _drDiff, _drFmtDate, _calcLsTotal, _hestiaFindAlternatives, _hestiaSendLead, _hestiaSendLeadFd, _hestiaFlushLeads });
...
const DIRECT_PERKS = {
  es: [
    ...
    { id:'proceso',   icon:'📜', stat:'20%',     t:'Contrato, prereserva y resto a convenir.',         d:'Te enviamos un borrador de contrato con derechos y obligaciones de ambas partes (precios, pagos, condiciones de cancelación y normas). Lo revisas, lo rellenas, lo firmas y nos lo devuelves. Una pequeña prereserva a convenir, normalmente el 20 % del total, se paga al formalizar el contrato; el resto, también a convenir, al llegar a Hestía. Acusamos recibo de todo (contrato y pago) para darte confianza, garantía y seguridad en cada paso.' },
  ],
  en: [
