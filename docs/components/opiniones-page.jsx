...
const CATEGORY_DEFS = [
  ...
  { id: 'zona', es: 'Zona y ubicación', en: 'Location', bias: 0,
...
// lista de debajo.
const CategoryBars = ({ reviews, lang }) => {
  if (!reviews.length) return null;
  const scores = categoryScores(reviews);
  return (
    <div className="opi-cat-bars reveal">
      <div className="opi-cat-title">{lang === 'es' ? 'Por categoría' : 'By category'}</div>
      <div className="opi-cat-list">
        {scores.map(cat => (
      ...
      <p className="opi-cat-note">
        {lang === 'es'
          ? 'Calculado a partir de lo que cuentan las reseñas reales de este filtro, no son sub-notas que den las plataformas.'
          : "Calculated from what this filter's real reviews actually say, not category sub-scores provided by the platforms."}
      </p>
    </div>
  );
};
  ...
  return (
    <section className="opiniones-testimonials">
      <div className="container">
        ...
        <CategoryBars reviews={filtered} lang={lang} />
