...
const AptCalendar = ({
  ...
  const handleDayClick = ds => {
    ...
    if (nights < effMin) {
      ...
      setSelMsg({
        type: 'error',
        es: isCrit ? `Temporada crítica: estancia mínima ${effMin} noches en estas fechas. Excepción: si rellenas exactamente un hueco entre dos reservas o si el check-in es esta semana (≤${imminentDays} días), bastarían 2 noches.` : `Estancia mínima ${effMin} noches. Selecciona una fecha de salida más adelante.`,
        en: isCrit ? `Critical season: minimum stay ${effMin} nights on these dates. Exception: if you exactly fill a gap between two bookings or check-in is this week (≤${imminentDays} days), 2 nights would suffice.` : `Minimum stay ${effMin} nights. Pick a later check-out date.`
      });
      return;
    }
    setSelEnd(ds);
