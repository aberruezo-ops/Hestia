...
const ReservasTab = ({
  ...
  const _syncDirectToAvailability = async allReservas => {
    ...
    for (const r of allReservas) {
      ...
      if (!_isCxl(r) && _isDirect(r)) newDirect[apt].push({
        start: r.entrada,
        end: r.salida
      });else if (_isCxl(r) && !_isDirect(r)) newExceptions[apt].push({
        start: r.entrada,
        end: r.salida
      });else if (!_isCxl(r) && !_isDirect(r)) activePaid[apt].push({
        start: r.entrada,
        end: r.salida
      });
    }
    ...
    const _subtract = (blocked, exceptions) => {
      ...
      for (const exc of exceptions) {
        const next = [];
        for (const b of res) {
          if (exc.end <= b.start || exc.start >= b.end) {
            next.push(b);
            continue;
          }
          if (exc.start > b.start) next.push({
            start: b.start,
            end: exc.start
          });
          if (exc.end < b.end) next.push({
            start: exc.end,
            end: b.end
          });
        }
    ...
    for (const apt of APTS_LIST) {
      if (newExceptions[apt].length && activePaid[apt].length) newExceptions[apt] = _subtract(newExceptions[apt], activePaid[apt]);
    }
